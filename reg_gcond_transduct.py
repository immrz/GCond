...
class RegGCond(GCond):
    ...
    def train(self, verbose=True):
        print(f"Training with SP regularization: lambda={self.adv_lambda}.")
        args = self.args
        data = self.data
        ...
        for it in range(args.epochs+1):
            ...
            for ol in range(outer_loop):  # second loop - train for this many steps
                ...
                for c in range(data.nclass):  # third loop - process each class separately
                    ...
                    batch_ys.append(labels[n_id[:batch_size]])
                ...
                # regularization by statistical parity
                outputs = torch.cat(outputs, dim=0)
                batch_gids = torch.cat(batch_gids, dim=0).cuda()
                batch_ys = torch.cat(batch_ys, dim=0).cuda()
                preds = outputs[range(outputs.shape[0]), batch_ys].exp()  # the 'soft' predictions; exp because outputs are log softmax
                loss_sp = 0.0
                for gid in torch.unique(batch_gids):
                    group_mask = (batch_gids == gid)
                    max_diff = -1
                    for y in range(data.nclass):
                        y_mask = (batch_ys == y)
                        diff = torch.abs(preds[y_mask].mean() - preds[y_mask & group_mask].mean())
                        if diff > max_diff:
                            max_diff = diff
                    assert max_diff >= 0
                    loss_sp += max_diff
                loss_sp /= ngroup
                (self.adv_lambda * loss_sp).backward()
